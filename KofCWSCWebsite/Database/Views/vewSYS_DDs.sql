...
create VIEW vewSYS_DDs
AS
--select * from vewSYS_DDs where year = dbo.funSYS_GetBegFratYearN(0) order by District

SELECT [dbo].[funSYS_GetDDForDistYear](dy.District,dy.Year) AS MemberID,ISNULL(mm.FirstName+' '+mm.LastName,'Vacant') as FullName,dy.Year,dy.District,mm.Council
