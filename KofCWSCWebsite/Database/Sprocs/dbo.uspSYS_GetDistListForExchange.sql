CREATE PROCEDURE [dbo].[uspSYS_GetDistListForExchange]
@Type NVARCHAR (10)='AGWM', @GroupID INT, @OfficeID INT, @NextYear INT=0
AS
BEGIN

-- exec [uspSYS_GetDistListForExchange] 'AGWM',0,0,0
-- exec [uspSYS_GetDistListForExchange] 'AGWM',0,13,0
----------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------
    ...
    IF @Type = 'AGWM'
        BEGIN
            PRINT 'ALL';
            SELECT   LEFT(@Prefix + CASE WHEN vo.groupid = 3 THEN 'DD' + dbo.funSYS_PaddWZeros(CAST (cmo.DISTRICT AS VARCHAR), 2) WHEN vo.OfficeID = 22 THEN 'FS' + CAST (mm.council AS VARCHAR) WHEN vo.OfficeID = 27 THEN 'GK' + CAST (mm.council AS VARCHAR) WHEN vo.OfficeID = 20 THEN 'FN' + CAST (mm.Assembly AS VARCHAR) WHEN vo.OfficeID = 16 THEN 'All Field Agents' WHEN vo.GroupID = 8 THEN 'All Past State Deputies' WHEN vo.OfficeID = 17 THEN 'FC' + CAST (mm.Assembly AS VARCHAR) ELSE vo.OfficeDescription END, 64) AS GroupName,
                     @Prefix + CASE WHEN vo.groupid = 3 THEN 'DD' + CAST (cmo.DISTRICT AS VARCHAR) + '@kofc-wa.org' WHEN vo.OfficeID = 16 THEN 'AllFAs@kofc-wa.org' WHEN vo.groupid = 8 THEN 'AllPSDs@kofc-wa.org' WHEN vo.OfficeID = 22 THEN 'FS' + CAST (mm.council AS VARCHAR) + '@kofc-wa.org' WHEN vo.OfficeID = 27 THEN 'GK' + CAST (mm.council AS VARCHAR) + '@kofc-wa.org' WHEN vo.OfficeID = 20 THEN 'FN' + CAST (mm.Assembly AS VARCHAR) + '@kofc-wa.org' WHEN vo.OfficeID = 17 THEN 'FC' + CAST (mm.Assembly AS VARCHAR) + '@kofc-wa.org' ELSE vo.EmailAlias + '@kofc-wa.org' END AS GroupEmail,
                     mm.FirstName + ' ' + mm.LastName AS RecipientName,
                     mm.Email AS RecipientEmail,
                     @ManagedBy AS ManagedBy,
                     CASE WHEN vo.groupid = 3 THEN 'DD' + CAST (cmo.DISTRICT AS VARCHAR) WHEN vo.OfficeID = 22 THEN 'FS' + CAST (mm.council AS VARCHAR) WHEN vo.OfficeID = 27 THEN 'GK' + CAST (mm.council AS VARCHAR) WHEN vo.OfficeID = 20 THEN 'FN' + CAST (mm.Assembly AS VARCHAR) WHEN vo.OfficeID = 17 THEN 'FC' + CAST (mm.Assembly AS VARCHAR) WHEN vo.OfficeID = 16 THEN 'FA' + mm.FirstName + mm.LastName ELSE vo.EmailAlias END AS Alias
    ...
    IF @Type = 'AGWG'
        BEGIN
            ...
            FROM   (SELECT @Prefix + 'All District Deputies' AS GroupName,
                           ...
                           @Prefix + 'DD' + CAST (cmo.District AS NVARCHAR) AS RecipientName,
                           @Prefix + 'DD' + CAST (cmo.District AS NVARCHAR) + '@KOFC-WA.ORG' AS RecipientEmail,
                           @ManagedBy AS ManagedBy,
